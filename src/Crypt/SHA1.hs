...
padding msg len = case length msg of
                     ...
                     size -> let len8      = len * 8
                             ...
                             in if size >= 56 
                                    then Left (msg ++ [0b10000000] ++ replicate (63 - size) (toEnum 0)
                                                , replicate 56 (toEnum 0) ++ endPart)
                                    else Right $ msg ++ [0b10000000] ++ replicate (55 - size) (toEnum 0) ++ endPart 
...
chunks msg = if BS.null msg 
                then case padding (BS.unpack msg) 0 of Right a -> [a]
                else chunks' msg $ BS.length msg
    where
        chunks' m l
            | BS.null m = []
...
sha1 :: [Word32] -> [Word32]
sha1 msg = let w = wrds msg (drop 13 msg) (drop 8 msg) (drop 2 msg) msg 16
    ...
    where
        ...
        processMsg [a,b,c,d,e] (w:ws) time = let t = time `div` 20
                                                 temp =  (a `rotateL` 5) |+|
...
msgDiggest msg = (map fromWord8List . cut 4) =<< msg

sha1Encoding :: BS.ByteString -> BS.ByteString
sha1Encoding = BS.pack . map wordToDigit . join . join . map (map hexPair . toWord8List) . sha1 . msgDiggest . chunks 
    where 
